<script setup>
    ...
    const data = ref({
        ...
        note: props.placement.note || "",
        supervisor_id: props.placement.supervisor_id || "0",
    });
    const errors = ref({
        note: "",
        supervisor_id: "",
    ...
    const push = async () => {
        ...
        const postUrl = `/placements/${data.value.id}/`;
        emits('onLoad', true);
        ...
        const content = resp.status == 200 ? `Data penempatan #${props.placement.id} berhasil diedit !` : json.detail;
        emits("toast", {
            "title": json.message,
            "content": content,
            "status": resp.status,
...
<template>
    <div class="mb-3">
        ...
        >
            ...
            <option 
                :value="supervisor.supervisor_profile.id" 
                v-for="supervisor in supervisors" 
                :key="supervisor.supervisor_profile.id"
            >
