<script setup>
    ...
    const admin = ref({
        ...
        password: "",   
        admin_profile: {
            fullname: props.data.admin_profile.fullname ? props.data.admin_profile.fullname : "",
            regency_id: props.data.admin_profile.regency_id ? props.data.admin_profile.regency_id : 0,
            full_address: props.data.admin_profile.full_address ? props.data.admin_profile.full_address : "",
            birth_place: props.data.admin_profile.birth_place ? props.data.admin_profile.birth_place : "",
            birth_date: props.data.admin_profile.birth_date ? new Date(props.data.admin_profile.birth_date).toISOString().slice(0,10) : Date.now(),
            phone: props.data.admin_profile.phone ? props.data.admin_profile.phone : "",
        }
    });
    ...
    const push = async () => {
        ...
        const body = JSON.stringify(admin.value);
        const postUrl = `/users/${admin.value.id}/`;
        emits('onLoad', true);
        ...
        const content = resp.status == 200 ? `Data pengguna admin ${admin.value.admin_profile.fullname} berhasil diedit !` : json.detail;
        emits("toast", {
            "title": json.message,
            "content": content,
            "status": resp.status,
...
<template>
    <CForm class="px-4 py-4">
        <div class="mb-3">
            ...
            <CFormInput readonly disabled :class="errors.email ? 'border-danger' : ''" v-model="admin.email" type="text" id="email" placeholder="Email pengguna"/>
            <div class="text-end text-danger" v-if="errors.email">
                <small>{{ errors.email }}</small>
            </div>
        </div>
        <div class="mb-3">
            <CFormLabel for="password">Password Baru</CFormLabel>
        ...
        <div class="mb-3">
            ...
            <CFormInput :class="errors.phone ? 'border-danger' : ''" v-model="admin.admin_profile.phone" type="text" id="phone" placeholder="Tanggal Lahir Pengguna"/>
