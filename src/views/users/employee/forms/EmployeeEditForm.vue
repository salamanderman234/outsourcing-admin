<script setup>
    ...
    const push = async () => {
        ...
        const body = JSON.stringify(employee.value);
        const postUrl = `/users/${employee.value.id}/`;
        emits('onLoad', true);
        ...
        const content = resp.status == 200 ? `Data karyawan berhasil diedit !` : json.detail;
        emits("toast", {
            "title": json.message,
            "content": content,
            "status": resp.status,
...
<template>
    <CForm class="px-4 py-4">
        ...
        <div class="mb-3">
            ...
            <CFormInput :class="errors.phone ? 'border-danger' : ''" v-model="employee.employee_profile.phone" type="text" id="phone" placeholder="Tanggal Lahir Pengguna"/>
