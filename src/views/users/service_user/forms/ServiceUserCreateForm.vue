<script setup>
    ...
    const push = async () => {
        ...
        const postUrl = `/service_user/register/`;
        emits('onLoad', true);
        ...
        const content = resp.status == 200 ? "Data pengguna jasa berhasil dibuat !, silahkan cek email yang terdaftar untuk memverifikasi pengguna" : json.detail;
        emits("toast", {
            "title": json.message,
            "content": content,
            "status": resp.status,
